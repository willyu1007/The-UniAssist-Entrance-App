...
4. Runtime persistence + teaching flow artifacts
...
- 扩 `packages/workflow-contracts`，补 teaching validation payload、run/artifact 查询 shape、`capturedRecipeDrafts` 响应字段，以及 compat executor 保留输入封套。
...
- 补 `docs/scenarios/teaching` 的 canonical workflow 说明与 fixture。
...
- Risk: teaching sample 反向定义平台基线
  - Mitigation: teaching-specific 语义放在 payload / scenario docs，不新增教学专属表
