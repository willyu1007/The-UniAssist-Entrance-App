...
## Status
- State: in-progress
- Status note: `T-018` 的高影响数据面边界已冻结，可作为 builder、historical sample、console、governance 和 bridge 设计的 authoritative baseline；本子包仍不承接 schema 实施。
...
- 不定义 historical sample 场景 parser / prompt 内部实现

## Context
- `T-011` 已锁定全局方向：Postgres/Prisma 继续作为主账本，`Convex` 不进入首期主数据面；历史上的首个 sample validation 场景需要 `fan-out` 与个性化评估 agent 收敛。
- `T-012` 已冻结服务骨架和 `P1` 最小核心对象集：`workflow/run/node/artifact/approval`。
- 后续 `builder-draft`、historical sample scenario design、`control-console` 都依赖统一的数据归属判断，如果不先冻结 authoritative/projection 边界，会在不同子包里重复发散。
